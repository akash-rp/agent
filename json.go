	...
	for _, frontend := range obj.Sites {
		...
		for _, alias := range frontend.AliasDomain {
			if alias.Routing == "www" {
				conf = conf + fmt.Sprintf(` redirect prefix http://www.%s code 301 if { hdr(host) -i %s }`, alias.Url, alias.Url)
			}
			if alias.Routing == "root" {
				conf = conf + fmt.Sprintf(` redirect prefix http://%s code 301 if { hdr(host) -i www.%s }`, alias.Url, alias.Url)
			}
		}
	}
